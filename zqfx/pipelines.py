...
class ZqfxPipeline(object):
    def __init__(self):
        dbargs = dict(host='127.0.0.1', db='mysite', user='root',  # replace with you user name
    ...
    def insert_into_table_fox008(self, conn, item):
        conn.execute('insert into zqfx_zqfx(datec,cc,fxs_leauge,fxs_leauge_name0,fxs_leauge_name1,fxs_2_02_c01,'
    ...
    def insert_into_table_live500(self, conn, item):
        conn.execute('insert into zqfx_live500(datec,cc,result,w,p,l,mar_right60,detail,live500_url) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)', (
            item['date'], item['cc'], item['result'], item['w'], item['p'], item['l'], item['mar_right60'],
            item['detail'],item['live500_url']))

    def insert_into_table_leisu(self, conn, item):
        conn.execute(
            'insert into zqfx_leisu(datec,team1,win,team2,lose,lsjf_left,lsjf_right,jqzj_left,jqzj_right,remarks,cc,url,information) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
            (item['date'], item['team1'], item['win'], item['team2'], item['lose'], item['lsjf_left'],
             item['lsjf_right'], item['jqzj_left'], item['jqzj_right'], item['remarks'], item['cc'], item['url'],item['information']))

    def insert_into_table_analysis(self, conn, item):
        conn.execute(
            "insert into zqfx_tips(datec,cc,wjxs,slfx_left,slfx_right,sjfx_left,sjfx_rigft,wjjk_left,wjjk_rigft,op,yp,dzwj,tencjk,pmxd,url,home,guest) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            (item['date'], item['cc'], item['wjxs'], item['slfx_left'], item['slfx_right'], item['sjfx_left'],
             ...
             item['_10cjk'], item['pmxd'],item['url'],item['home'],item['guest']))

    def insert_into_table_sporttery(self, conn, item):
        conn.execute(
            "insert into zqfx_sporttery(datec,cc,league,zhu,ke,half,full,w,p,l,had_prs_name,had_odds,hhad_prs_name,hhad_odds,crs_prs_name,crs_odds,ttg_prs_name,ttg_odds,hafu_prs_name,hafu_odds,had_h_rate,had_d_rate,had_a_rate,same_result,detail) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
    ...
    def insert_into_table_now(self, conn, item):
        conn.execute(
            "insert into zqfx_now1(datec,cc,league,zhu,ke,half,full,w,p,l,had_prs_name,had_odds,hhad_prs_name,hhad_odds,crs_prs_name,crs_odds,ttg_prs_name,ttg_odds,hafu_prs_name,hafu_odds,had_h_rate,had_d_rate,had_a_rate,same_result,detail) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
            (item['date'], item['cc'], item['league'], item['zhu'], item['ke'], item['half'], item['full'], item['w'],
             item['p'], item['l'], item['had_prs_name'], item['had_odds'], item['hhad_prs_name'], item['hhad_odds'],
             item['crs_prs_name'], item['crs_odds'], item['ttg_prs_name'], item['ttg_odds'], item['hafu_prs_name'],
             item['hafu_odds'], item['had_h_rate'], item['had_d_rate'], item['had_a_rate'], item['same_result'],
    ...
    def insert_into_table_win007(self, conn, item):
        conn.execute(
            "insert into zqfx_win007(datec,cc,mar_right60,detail,url,mar_right1,mar_right2,mar_right3,mar_right4,mar_right5,mar_right6, match_time, home_team, guest_team, h_odd, pk_odd, g_odd, full_result,oddstr_281_f_result,oddstr_281_l_result,oddstr_115_f_result,oddstr_115_l_result,oddstr_82_f_result,oddstr_82_l_result,odds_3_f_result,odds_3_l_result,odds_8_f_result,odds_8_l_result,odds_14_f_result,odds_14_l_result) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)",
    ...
    def del_table_fox008(self, conn, item):
        sql = "DELETE  FROM zqfx_zqfx where datec = '" + item['date'] + "' and cc = '" + item['cc'] + "'"
        conn.execute(sql)

    def del_table_sporttery(self, conn, item):
        sql = "DELETE  FROM zqfx_sporttery where datec = '" + item['date'] + "' and cc = '" + item['cc'] + "'"
        conn.execute(sql)

    def del_table_now(self, conn, item):
        sql = "DELETE  FROM zqfx_now1  where datec = '" + item['date'] + "' and cc = '" + item['cc'] + "'"
        conn.execute(sql)

    def del_table_live500(self, conn, item):
        sql = "DELETE  FROM zqfx_live500 where datec = '" + item['date'] + "' and cc = '" + item['cc'] + "'"
        conn.execute(sql)

    def del_table_analysis(self, conn, item):
        sql = "DELETE  FROM zqfx_tips where datec = '" + item['date'] + "' and cc = '" + item['cc'] + "'"
        conn.execute(sql)

    def del_table_leisu(self, conn, item):
        sql = "DELETE  FROM zqfx_leisu where datec = '" + item['date'] + "' and team1 = '" + item['team1'] + "'"
        conn.execute(sql)

    def insert_into_table_vipc(self, conn, item):
        conn.execute(
            'insert into zqfx_vipc(datec,cc,win,lose,result,bf,url,home,guest,content,matchId,ratio,num,betting,suggest0,letgoal,suggest1) values(%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)',
            (
    ...
    def del_table_vipc(self, conn, item):
        sql = "DELETE  FROM zqfx_vipc where datec = '" + item['date'] + "' and cc = '" + item['cc'] + "'"
        conn.execute(sql)
